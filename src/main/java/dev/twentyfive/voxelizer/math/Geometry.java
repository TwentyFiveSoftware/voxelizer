...
public class Geometry {
    ...
    public static ArrayList<Voxel> voxelizeModel(Model model, double thickness) {
        ...
        for (Triangle triangle : model.triangles) {
            ...
            Vector3 projectedA = projectVector3on2DPlaneCoordinates(planeNormal, a, b, a);
            Vector3 projectedB = projectVector3on2DPlaneCoordinates(planeNormal, a, b, b);
            Vector3 projectedC = projectVector3on2DPlaneCoordinates(planeNormal, a, b, c);
            ...
            for (int x = minX; x <= maxX; x++) {
                for (int y = minY; y <= maxY; y++) {
                    for (int z = minZ; z <= maxZ; z++) {
                        ...
                        double distance = Math.abs(calculateSignedDistanceBetweenPlaneAndPoint(planeNormal, a, point));
                        if (distance > thickness)
                            continue;

                        Vector3 projectedPoint = projectVector3on2DPlaneCoordinates(planeNormal, a, b, projectPointOnPlane(planeNormal, a, point));
    ...
    private static double calculateSignedDistanceBetweenPlaneAndPoint(Vector3 planeNormal, Vector3 planePoint, Vector3 point) {
    ...
    private static Vector3 projectVector3on2DPlaneCoordinates(Vector3 planeNormal, Vector3 planePointA, Vector3 planePointB, Vector3 point) {
